...
@Component({
  selector: 'app-tenent-user-edit',
  templateUrl: './tenent-user-edit.component.html',
  styleUrls: ['./tenent-user-edit.component.css']
})
export class TenentUserEditComponent extends RoleAuthorizerUtility implements OnInit {
  ...
  secRoleForm;
  secUserLocation;
  public ShowSaveButton: boolean = this.isAuthorize([Roles.ADMIN, Roles.MAINTAINER_OFFICER]);
  ...
  async ngOnInit(): Promise<void> {
    ...
    //if (this.DisableFields) this.UserForm.disable();
    this.loadPrefix();
    ...
    await this.AllBuildings();
    const { buildingId, floorId } = this.SecUserData as any;
    debugger;
    await this.loadFloors(buildingId); 
    debugger;
    await this.loadFlats(floorId);
    //this.loadAllAgency();
    this.FillFields();
  }
  hideShowPass() {
  ...
  private async LoadUserData(): Promise<void> {
    if (this.userId == null) return; //! Exit If UserId is null
    ...
    if (com != undefined && com != null) {
      ...
      this.SecUserService.GetUserbyId(this.id).subscribe((data) => {
        ...
        if (this.userUpdateId != undefined) {
          ...
          if (
            this.UserMaker.parentAgencyId != null &&
            this.UserMaker.parentAgencyId != undefined
          ) {
            this.UserForm.get("ParentAgencyId").setValue(
              this.UserMaker.parentAgencyId
            );
            this.loadAllUsers(this.UserMaker.parentAgencyId);
            this.UserForm.get("ParentUserId").setValue(
              this.UserMaker.parentAgencyName
            );
          }
          ...
          let req = new Date(
            this.datePipe.transform(this.UserMaker.dateOfBirth, "yyyy/MM/dd")
          );
          // this.LibraryForm.controls.Uploaddate.setValue(LBDate);
          this.UserForm.get("DateofBirth").setValue(
            this.datePipe.transform(req, "yyyy-MM-dd")
          );
          // this.UserForm.get('DateofBirth').setValue(this.UserMaker.dateOfBirth);
          this.UserForm.get("RegistrationNo").setValue(
            this.UserMaker.registrationNo
          );
          ...
          this.UserForm.get("EmailForgotPassword").setValue(
            this.UserMaker.emailForgotPassword
          );
  ...
  onRecordSubmit(): void {
    ...
    this.submitted = true;
     ...
     this.UserForm.get("BuildingId").setValue(this.SecUserData["buildingId"]);
    ...
    if (this.id != undefined && this.id != null && this.id > 0) {
      ...
      if (
        ...
        this.UserForm.get("Password").value == undefined ||
      ...
      if (
        ...
        this.UserForm.get("ConfirmPassword").value == undefined ||
    ...
    foData.append("createdByUser", "true");
    ...
    if (
      this.UserForm.get("Mobile").value != null &&
      this.UserForm.get("Mobile").value != undefined &&
    ...
    if (
      this.UserForm.get("CountryId").value != null &&
      this.UserForm.get("CountryId").value != undefined &&
      this.UserForm.get("CountryId").value != "" &&
      this.UserForm.get("CountryId").value != ""
    ) {
    ...
    if (
      this.UserForm.get("StateId").value != null &&
      this.UserForm.get("StateId").value != undefined &&
      this.UserForm.get("StateId").value != "" &&
      this.UserForm.get("StateId").value != ""
    ) {
    ...
    if (
      this.UserForm.get("CityId").value != null &&
      this.UserForm.get("CityId").value != undefined &&
      this.UserForm.get("CityId").value != "" &&
      this.UserForm.get("CityId").value != ""
    ) {
    ...
    foData.append(
      "EmailForgotPassword",
      this.UserForm.get("EmailForgotPassword").value
    );
    //zeeshan
    if (
      this.UserForm.get("BuildingId").value != null &&
      this.UserForm.get("BuildingId").value != undefined &&
      this.UserForm.get("BuildingId").value != "" &&
      this.UserForm.get("BuildingId").value != ""
    ) {
    ...
    if (
      this.UserForm.get("FloorId").value != null &&
      this.UserForm.get("FloorId").value != undefined &&
      this.UserForm.get("FloorId").value != "" &&
      this.UserForm.get("FloorId").value != ""
    ) {
    ...
    if (
      this.UserForm.get("FlatId").value != null &&
      this.UserForm.get("FlatId").value != undefined &&
      this.UserForm.get("FlatId").value != "" &&
      this.UserForm.get("FlatId").value != ""
    ) {
    ...
    //}
    debugger;
    this.SecUserService.UserCreateWithFiles(foData).subscribe(
      async (Response) => {
        debugger;
        ...
        if (Response.message == "0") {
          ...
          await this._tenantBuilding.UpdateFlats(
            this.UserForm.get("FlatId").value,
            { isAllotted: true }
          );
          this.router.navigate([`/app/userManagement`]);
  ...
  private FillFields(): void {
    ...
    debugger;
    this.UserForm.patchValue(this.SecUserData);
  }
  loadLocations(): void {
  ...
  async loadFloors(buildingId): Promise<void> {
    ...
    var BuildingData = this.BuildingList.find(
      (item) => item.id === parseInt(buildingId)
    );
    this.UserForm.get('BuildingId').setValue(BuildingData['name']);
    console.log(BuildingData);
    this.UserForm.get("CountryId").setValue(BuildingData["countryId"]);
    this.UserForm.get("CityId").setValue(BuildingData["cityId"]);
    this.UserForm.get("StateId").setValue(BuildingData["stateId"]);
    this.UserForm.get("CountryName").setValue(BuildingData["countryName"]);
    this.UserForm.get("CityName").setValue(BuildingData["cityName"]);
    this.UserForm.get("StateName").setValue(BuildingData["stateName"]);
    this.UserForm.get("EmailForgotPassword").setValue(8);
  }
  async loadUserManagement(): Promise<void> {
    this.User = await this._tenantUserManagement.GetUserId(this.userId) as any;
  }
  async loadFlats(floorsId): Promise<void> {
    debugger;
    this.flatsList = await this._tenantBuilding.GetAllFlatUpdate([
      {
        QueryParam: "FloorId",
        value: floorsId,
        method: ParamMethod.FILTER,
  ...
  async onAddUserManagementFormSubmit(): Promise<void> {
    ...
    const UpdateClient = { ...this.UserForm.value };
