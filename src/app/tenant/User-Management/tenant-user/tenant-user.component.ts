...
export class TenantUserComponent implements OnInit {
  ...
  async AllBuildings(): Promise<void> {
    ...
    await this.loadFloors(this.SecUserData["buildingId"]); 
  }
  ...
  UserSubmit(): void {
    debugger;
    if (
      this.UserForm.get("NewConfirmPassword").value !=
    ...
    if (
      this.UserForm.get("EmailForgotPassword").value == null ||
      this.UserForm.get("EmailForgotPassword").value == undefined ||
      this.UserForm.get("EmailForgotPassword").value == ""
    ) {
  ...
  DownloadConfidentially(): void {
    if (this.Confidentialitypath == true) {
      ...
      this._toster.error("File Not Exsit", "Alert");
    }
  }
