...
import { EaCodeModel } from '@shared/Dto/Ea-Code-model';
import {EaCodeService} from 'shared/Services/Ea-Code-service';

import { ToastrService } from 'ngx-toastr';
@Component({
  selector: 'app-ea-code',
  templateUrl: './ea-code.component.html',
  styleUrls: ['./ea-code.component.css']
})
export class EaCodeComponent implements OnInit {

  public item: EaCodeModel = new EaCodeModel();
  EaCodeForm = new FormGroup({
    // Id: new FormControl(''),
    Id: new FormControl(''),
 ...
 get f() { return this.EaCodeForm.controls; }
  ...
  constructor(service: EmployeesService,
    ...
    private route: ActivatedRoute,
     public EaCodeService: EaCodeService,
    ...
    // {
     ...
     // this.ModuleForm.controls.Code.setValue(e.row.data.code);
      this.EaCodeForm.controls.Id.setValue(e.row.data.id);
      this.EaCodeForm.controls.Name.setValue(e.row.data.name);
      this.EaCodeForm.controls.Code.setValue(e.row.data.code);
      this.EaCodeForm.controls.Description.setValue(e.row.data.description); 
    ...
    // this.CertificateForm.controls.StartDate.setValue(e.row.data.startdate);
      ...
      this.EaCodeForm.controls.IsActive.setValue(1);
      }else{this.EaCodeForm.controls.IsActive.setValue(0);}
...
delete(e) {
    ...
    //                this.EaCodeService.Delete(e.row.data.id).subscribe((Response)=>{
    ...
    onTableSizeChange(event): void {
      ...
      onSubmit(): void {
        ...
        this.item=new EaCodeModel();
        this.submitted = true;
    
        // stop here if form is invalid
        if (this.EaCodeForm.invalid) {
    ...
    this.item.Name=this.EaCodeForm.get('Name').value
    this.item.Code=this.EaCodeForm.get('Code').value
    this.item.Description=this.EaCodeForm.get('Description').value
    
    if(this.EaCodeForm.get('IsActive').value==1)
    {
    ...
    // this.item.IsDeleted=this.CertificateForm.get('IsDeleted').value
          ...
          this.EaCodeService.create(this.item).subscribe((Response)=>{
    ...
    onSearch(){
      ...
      this.EaCodeService.Get(this.pagedDto).subscribe((Response) => {
                  
      
        this.totalCount = Response.totalCount
        this.EaCodeList = Response.eaCodeModel
      })
    }
    ...
    NewRecord()
     ...
     {  
       this.id=0;
      this.EaCodeForm.controls.Id.setValue('');
      this.EaCodeForm.controls.Name.setValue('');
      this.EaCodeForm.controls.Code.setValue('');
      this.EaCodeForm.controls.Description.setValue(''); 
    
    // this.CertificateForm.controls.StartDate.setValue(e.row.data.startdate);
      //let req = new Date(this.datePipe.transform(e.row.data.startDate, 'yyyy/MM/dd'))
    
       //this.CertificateForm.controls.StartDate.setValue(this.datePipe.transform(req, 'yyyy/MM/dd'))
    this.EaCodeForm.controls.IsActive.setValue('');
